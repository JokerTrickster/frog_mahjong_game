...
func WSHandleMessages(gameName string) {
	...
	go func() {
		for {
			...
			if err := rabbitManager.PublishMessage(gameName, msgBytes); err != nil {
				log.Printf("Failed to publish message to find-it queue: %v", err)
			}
		}
	}()
	...
	go func() {
		for {
			...
			if err != nil {
				log.Printf("Failed to get channel for find-it queue: %v", err)
			...
			if err != nil {
				log.Printf("Failed to register consumer for find-it queue: %v", err)
				time.Sleep(5 * time.Second) // 재시도 대기
				continue
			}

			log.Printf("Waiting for messages for game: %s (find-it)", gameName)
			for msg := range msgs {
