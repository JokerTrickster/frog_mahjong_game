...
func TimeOutDiscardCardsFindOneDora(c context.Context, tx *gorm.DB, roomID uint) (*mysql.FrogUserCards, error) {
...
func TimeOutDiscardCardsUpdateCardState(c context.Context, tx *gorm.DB, entity *entity.WSTimeOutDiscardCardsEntity) error {
	...
	err := tx.Model(&mysql.FrogUserCards{}).Where("room_id = ? and card_id = ? and state = ?", entity.RoomID, entity.CardID, "none").Updates(&mysql.FrogUserCards{State: "discard", UserID: int(entity.UserID)}).Error
	if err != nil {
...
func TimeOutDiscardCardsFindAllCard(c context.Context, tx *gorm.DB, roomID uint, userID uint) ([]*mysql.FrogUserCards, error) {
