...
func LoanCardFindOneDora(c context.Context, tx *gorm.DB, roomID uint) (*mysql.FrogUserCards, error) {
...
func LoanCardLoan(c context.Context, tx *gorm.DB, loanEntity *entity.WSLoanEntity) error {
	...
	err := tx.Model(&mysql.FrogUserCards{}).Where("room_id = ? AND user_id = ? and card_id = ? and state = ?", loanEntity.RoomID, loanEntity.TargetUserID, loanEntity.CardID, "discard").Updates(map[string]interface{}{"user_id": loanEntity.UserID, "state": "owned"}).Error
	if err != nil {
