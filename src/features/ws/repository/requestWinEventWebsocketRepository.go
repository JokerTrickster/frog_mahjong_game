...
func RequestWinFindOneDora(c context.Context, tx *gorm.DB, roomID uint) (*mysql.FrogUserCards, error) {
...
func RequestWinFindAllCards(c context.Context, tx *gorm.DB, requestWinEntity *entity.WSRequestWinEntity) ([]*mysql.FrogUserCards, error) {
	cards := make([]*mysql.FrogUserCards, 0)
	err := tx.Model(&mysql.FrogUserCards{}).Where("room_id = ? and user_id = ? and card_id IN ?", requestWinEntity.RoomID, requestWinEntity.UserID, requestWinEntity.Cards).Find(&cards).Error
	if err != nil {
...
func RequestWinDeleteAllCards(ctx context.Context, tx *gorm.DB, requestWinEntity *entity.WSRequestWinEntity) error {
	err := tx.Model(&mysql.FrogUserCards{}).Where("room_id = ?", requestWinEntity.RoomID).Delete(&mysql.FrogUserCards{}).Error
	if err != nil {
...
func RequestWinUpdateRoomUsers(c context.Context, tx *gorm.DB, requestWinEntity *entity.WSRequestWinEntity) error {
	err := tx.Model(&mysql.FrogRoomUsers{}).Where("room_id = ?", requestWinEntity.RoomID).Update("player_state", "wait").Error
	if err != nil {
