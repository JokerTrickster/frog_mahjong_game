...
func RandomUpdateRandomCards(c context.Context, tx *gorm.DB, randomEntity *entity.WSRandomEntity) error {
	...
	var cardIDs []int
	err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).WithContext(c).
...
func RandomUpdateRoomUserCardCount(c context.Context, tx *gorm.DB, entity *entity.WSRandomEntity) error {
	addedCount := fmt.Sprintf("owned_card_count + %d", entity.Count)
	// 유저id로 room_users에서 찾아서 card_count를 더한 후 업데이트 한다.
	err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Model(&mysql.RoomUsers{}).Where("room_id = ? AND user_id = ?", entity.RoomID, entity.UserID).Update("owned_card_count", gorm.Expr(addedCount)).Error
	if err != nil {
...
func RandomUpdateOpenCards(ctx context.Context, tx *gorm.DB, roomID uint) error {
...
func RandomUpdateAllCardState(c context.Context, tx *gorm.DB, roomID uint) error {
