...
func MatchEventWebsocket(msg *entity.WSMessage) *entity.ErrorInfo {
	...
	err := mysql.Transaction(mysql.GormMysqlDB, func(tx *gorm.DB) error {
		...
		// 게임 유저 정보 생성
		SequenceUserDTO := CreateMatchUserDTO(uID, roomID)
		errInfo = repository.MatchInsertUserDTO(ctx, tx, SequenceUserDTO)
		if errInfo != nil {
			return fmt.Errorf("%s", errInfo.Msg)
		}

		// 게임 룸 셋팅 생성
		SequenceGameRoomSettingDTO := CreateMatchGameRoomSettingDTO(roomID)
		errInfo = repository.MatchInsertGameRoomSettingDTO(ctx, tx, SequenceGameRoomSettingDTO)
		if errInfo != nil {
			return fmt.Errorf("%s", errInfo.Msg)
		}
