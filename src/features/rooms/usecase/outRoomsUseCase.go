...
func (d *OutRoomsUseCase) Out(c context.Context, uID uint, req *request.ReqOut) error {
	...
	err := mysql.Transaction(mysql.GormMysqlDB, func(tx *gorm.DB) error {
		...
		err := d.Repository.FindOneAndDeleteRoomUser(ctx, tx, uID, req.RoomID)
		if err != nil {
			return err
		}
		// Rooms 현재 인원수를 -1한다.
		roomDTO, err := d.Repository.FindOneAndUpdateRoom(ctx, tx, req.RoomID)
		if err != nil {
			return err
		}
		// user에 rooms_id를 1로 바꾸고 state를 wait으로 변경한다.
		err = d.Repository.FindOneAndUpdateUser(ctx, tx, uID)
		if err != nil {
			return err
		}
		...
		if roomDTO.CurrentCount == 0 {
			// 방 삭제
			err = d.Repository.FindOneAndDeleteRoom(ctx, tx, req.RoomID)
			if err != nil {
				return err
			}
			...
			//룸에 남아있는 유저 정보를 가져온다.
			roomUserDTO, err := d.Repository.FindOneRoomUser(ctx, tx, req.RoomID)
			if err != nil {
				return err
			}
			userDTO, err := d.Repository.FindOneUser(ctx, tx, uint(roomUserDTO.UserID))
			if err != nil {
				return err
			}
			...
			// rooms -> owner 변경
			err = d.Repository.ChangeRoomOnwer(ctx, tx, req.RoomID, userDTO.ID)
			if err != nil {
				return err
			}
