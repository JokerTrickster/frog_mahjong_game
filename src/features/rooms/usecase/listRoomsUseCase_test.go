...
// 테스트 경로: src/features/rooms/usecase/listRoomsUseCase_test.go

func TestListRoomsUseCase_List(t *testing.T) {
	now := time.Now()
	tests := []struct {
		name     string
		page     int
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// given
			mockRoomRepo := new(mocks.IListRoomsRepository)
			...
			uc := NewListRoomsUseCase(mockRoomRepo, 8*time.Second)
...
// TestCreateResListRoom 함수는 ListRoomsUseCase 의 CreateResListRoom 메서드를 테스트합니다.
// 테이블 기반 테스트를 사용하여 여러 시나리오를 테스트합니다.
// given-when-then 패턴을 사용하여 테스트를 작성합니다.
// 테스트 케이스:
// 1. 방 목록이 1개인 경우
// 2. 방 목록이 0개인 경우
// 테스트 경로: src/features/rooms/usecase/listRoomUseCase_test.go
