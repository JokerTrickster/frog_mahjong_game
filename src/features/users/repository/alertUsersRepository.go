...
func (d *AlertUsersRepository) FindOneAndUpdateUsers(ctx context.Context, userID uint, req *request.ReqAlertUsers) error {
	err := d.GormDB.WithContext(ctx).Model(&mysql.Users{}).Where("id = ?", userID).Update("alert_enabled", req.Alert).Error
	if err != nil {
